...
//from https://en.wikipedia.org/w/api.php?action=sitematrix&format=json NOT IN USE ANYMORE - CAN BE REMOVED
var site_map = {
  ...
  itvikidia: 'https://it.vikidia.org'
};
if (typeof module !== 'undefined' && module.exports) {
...
var makeUrl = function makeUrl(title, nwiki) {
  var url = 'https://' + nwiki + '/w/api.php';
...
var toHtml = _dereq_('./toHtml');
var toLatex = _dereq_('./toLatex');
var server = 'https://upload.wikimedia.org/wikipedia/';
var aliasList = _dereq_('../lib/aliases');
...
var methods = {
  ...
  url: function url(lang) {
    lang = lang || "commons";
    return server + lang + '/' + makeSrc(this.file);
  },
  thumbnail: function thumbnail(size, lang) {
    size = size || 300;
    lang = lang || "commons"
    var path = makeSrc(this.file);
    var title = encodeTitle(this.file);
    title = encodeURIComponent(title);
    return server + lang + '/thumb/' + path + '/' + size + 'px-' + title;
  },
...
var externals = {
  ...
  //'youtube': generic, -- youtube removed since we want to parse as generic
